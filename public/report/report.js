...
myObj = {
 ...
 document.getElementById('download').style.display = 'block';
  ...
  if (token) {
    ...
    try {
      await axios.post('http:// 44.198.130.123:3000/get/report', myObj, {
...
async function get_Report_Table (response, ispremium) {
 ...
 async function download(event) {
  ...
  const customAuthorizationHeader = `MyAuthHeader ${token}`
    console.log(token)
    axios
      .get('http:// 44.198.130.123:3000/get/expense/download', {
...
if (token) {
 const customAuthorizationHeader = `MyAuthHeader ${token}`
 axios.get('http:// 44.198.130.123:3000/get/expense/downloadlink', {headers: { Authorization: customAuthorizationHeader} })
.then((response ) => {
...
async function fetchDownloadLinks(page,itemsPerPage) {
  ...
  if (token) {
    const customAuthorizationHeader = `MyAuthHeader ${token}`;
    try {
      const response = await axios.get(`http:// 44.198.130.123:3000/get/expense/downloadlink?page=${page}&perPage=${itemsPerPage}`, {
